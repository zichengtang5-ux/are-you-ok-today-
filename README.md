...
面向独居人群的安全提醒 App。用户每天确认一次“平安”；若在提醒窗口结束后仍未确认，系统会进入宽限期，并在超时后通过推送、短信、语音电话通知紧急联系人。
...
│   ├── src/services/     # API、SSE、通知、IAP、深链、错误上报、定位
│   ├── src/store/        # Zustand 全局状态
│   └── src/components/   # UI 组件与主题
├── server/               # NestJS 后端
│   ├── src/              # auth、reply、reminder、alert、subscription 等模块
...
1. 用户完成注册、协议确认、地址定位/填写、联系人、提醒时间和通知授权。
...
7. 本期前端保持极简：不提供家庭守护、关怀看板、代确认和代付入口。
