...
| # | 事项 | 负责方 | 状态 | 备注 |
|---|------|--------|------|------|
| P1-1 | EAS Build 配置 | 前端 | ✅ 已完成 | `eas.json` 含 dev/preview/production profile，`app.json` 含 `extra.eas.projectId` 占位 |
| P1-2 | 安装 react-native-iap + 取消注释生产代码 | 前端 | ✅ 已完成 | 改用动态 require 模式，安装后自动生效，无需手动取消注释 |
| P1-3 | App Store Connect 配置订阅产品 | 产品/运营 | ⬜ 待做 | `com.todayok.subscription.monthly` + `yearly` |
| P1-4 | 环境变量配置 | 前端 | ✅ 已完成 | `.env` + `.env.production` 创建，`api.ts` 使用 `EXPO_PUBLIC_API_URL` |
| P1-5 | 用户协议 + 隐私政策真实 URL | 产品/法务 | ⬜ 待做 | 当前为 Alert 占位，需替换为真实 WebView 链接 |
| P1-6 | 推送通知 token 上报确认 | 前端 | ✅ 已完成 | 启动时 `registerDeviceToken()`，`addPushTokenListener` 监听 token 变化自动重注册 |
