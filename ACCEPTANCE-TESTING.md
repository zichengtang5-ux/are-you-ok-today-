...
- 紧急联系人不需要填写联系人验证码
- 提醒时间页可滚动选择 `00:00` 至 `23:00` 的开始/结束整点；`11:00` 表示上午，`23:00` 表示晚上；开始与结束不能相同
- 支持跨日窗口，例如 `23:00 - 次日 01:00`，跨日零点后的签到归属前一天守护周期
- 免费版最多 1 位联系人，守护版最多 5 位；联系人可添加、编辑和删除
- 免费版告警只产生主联系人的短信任务；守护版可通知最多 5 位联系人并增加语音电话任务；订阅过期后立即只返回并通知主联系人
...
- 首页保持已确认状态
- 同一天重复点击或通知动作重复提交时接口幂等成功，不重复累计天数
- 次日进入新守护日后，不会继续显示昨日已签到
...
- 恢复后状态回到可继续提醒的状态
- 设置页可看到暂停截止日期和恢复按钮
- 提前恢复和暂停自然到期后，`ReminderConfig.nextDueAt` 会重算到下一次有效提醒
...
- 后端失败时 StoreKit 交易保持待确认，可重试；“恢复购买”可找回已购订阅
- 同一 Apple 原始交易不能绑定两个账号，月付交易不能伪装成年付
...
- 有定位权限时提交经纬度
- 定位失败时可回退保存地址
- 定位结果自动填入一个可编辑的完整地址框，用户可在同一处补充楼栋、门牌和房间号
- 页面不显示 110 / 120 快捷拨号入口
- 后端创建 `HelpRequest`
- 每位联系人都有 `sent` 或 `failed` 通知日志，接口返回 `deliveryStatus`
- 部分失败时页面分别列出短信提交成功与失败联系人，并提供直接拨打入口
