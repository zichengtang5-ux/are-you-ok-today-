...
let iapModulePromise: Promise<typeof import('react-native-iap')> | null = null;

async function loadIapModule() {
  if (iapModulePromise) return iapModulePromise;
  iapModulePromise = import('react-native-iap').catch((err) => {
    throw new Error(
      `react-native-iap 未安装。请先执行 \`npx expo install react-native-iap\` 后使用 EAS Build 构建。原始错误：${
